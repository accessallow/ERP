...
class Seller_model extends CI_Model {
    ...
    function count_my_products($seller_id) {
        ...
        $queryString = "select m.id,m.product_id,m.seller_id,m.product_price,m.tag from
                        product_seller_mapping m,products p
                        where(
                            m.seller_id=$seller_id and
                            p.tag=1 and
                            m.tag=1 and
                            m.product_id = p.id
                        )";
